...
| **API-08** | API | AC-07, BR-03 | Requester ticket creation resolves `requesterId` from session | HTTP 201; ticket persisted with `requesterId` matching JWT user ID regardless of body | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| **API-09** | API | AC-08, BR-03 | Requester A attempts to fetch ticket belonging to Requester B | HTTP 403 Forbidden or 404 Not Found; zero data leaked | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| **API-10** | API | AC-21, FR-09 | Requester attempts to access IT Staff queue (`GET /api/staff/tickets`) | HTTP 403 Forbidden | `server/tests/lab-03/authorization.api.test.ts` | Pass |
| **API-11** | API | AC-21, FR-09 | Requester attempts to access Admin API (`GET /api/admin/users`) | HTTP 403 Forbidden | `server/tests/lab-03/authorization.api.test.ts` | Pass |
...
| **API-17** | API | AC-13, BR-14 | IT Staff transitions status `NEW` -> `IN_PROGRESS` | HTTP 200; status updated | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
| **API-18** | API | AC-13, BR-14 | Disallowed status transition (e.g., `NEW` -> `CLOSED`) | HTTP 422 Unprocessable Entity; status transition rejected | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
| **API-19** | API | AC-13, BR-15 | Transitioning to `RESOLVED` without `resolutionSummary` | HTTP 422 Unprocessable Entity; mandatory resolution summary required | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
...
| **API-30** | API | FR-26, BR-19 | Admin resets user initial password | HTTP 200; `mustChangePassword = true`; temporary password active | `server/tests/lab-03/users-admin.api.test.ts` | Pass |
...
| **E2E-02** | E2E | AC-02, AC-03 | Mandatory First-Login Password Change: Login with initial pass -> Enforced redirect -> Save new pass -> Main app | App is blocked until compliant password is saved | `e2e/lab-03/authentication.spec.ts` | Pass |
...
| **E2E-04** | E2E | AC-17, AC-19, AC-20 | Administrator User Management: Search users -> Create user -> Reset password -> Prevent self-deactivation | All administrative functions and safety guards verified | `e2e/lab-03/user-administration.spec.ts` | Pass |
...
| **AC-02** (Mandatory First-Login Password Change Guard) | `API-04`, `UI-03`, `E2E-02` | `server/tests/lab-03/auth.api.test.ts`<br>`client/tests/lab-03/ChangePassword.test.tsx`<br>`e2e/lab-03/authentication.spec.ts` |
| **AC-03** (Password Complexity Validation) | `API-05`, `API-06`, `UI-03`, `E2E-02` | `server/tests/lab-03/auth.api.test.ts`<br>`client/tests/lab-03/ChangePassword.test.tsx`<br>`e2e/lab-03/authentication.spec.ts` |
...
| **AC-07** (Requester Identity Binding) | `API-08` | `server/tests/lab-03/authorization.api.test.ts` |
| **AC-08** (Requester Ticket Isolation) | `API-09` | `server/tests/lab-03/authorization.api.test.ts` |
| **AC-09** (Internal Notes Protection from Requesters) | `API-22` | `server/tests/lab-03/comments-notes.api.test.ts` |
...
| **AC-21** (Role-Based Endpoint Protection) | `API-10`, `API-11` | `server/tests/lab-03/authorization.api.test.ts` |
