...
| **API-33** | API | AC-25, FR-12 | Attachment upload, download, and soft-remove under authenticated session | HTTP 201/200; cross-requester access returns 404 | `server/tests/lab-03/authorization.api.test.ts` | Pass |
...
| **API-35** | API | BR-22, FR-12 | Database migration integrity (Lab 2 tickets & attachments remain linked) | HTTP 200; historical data intact | `server/tests/lab-03/authorization.api.test.ts` | Pass |
