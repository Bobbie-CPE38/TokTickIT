...
export function validateNoteInput(input: unknown): ValidationResult {
  ...
  if (content === undefined || content === null || typeof content !== "string") {
    details.push("Note content is required and must be a string.");
    return { isValid: false, details };
  }
  ...
  if (trimmed.length === 0) {
    details.push("Note content must not be empty or whitespace-only.");
  } else if (trimmed.length > 2000) {
    details.push("Note content must not exceed 2,000 characters.");
  }
